...
def findSimilar(scanner1P, scanner2P, scanner1, scanner2):
    ...
    #could be in any of the 24 directions
    #you have offsets, and then you can swap any 2 points
    maxTotal = 0
    for offset in final:
        x = abs(offset[0])-1
        y = abs(offset[1])-1
...
def checkOverlap(scanner1, scanner2):
    ...
    for v in scanner1:
        for v2 in scanner2:
            if findSimilar(v,v2,scanner1,scanner2):
                print(v)
                print(v2)
                return findSimilar(v,v2,scanner1,scanner2)
    return False
...
for scanner1 in range(0,len(scanners)):
    for scanner2 in range(0, len(scanners)):
        if scanner1 == scanner2 or realPositions[scanner2] != "?":
            continue
        ...
        if c:
            ...
            try:
                ...
                realPositions[scanner2] = [s1Pos[0] + c[0][0], s1Pos[1] + c[0][1], s1Pos[2] + c[0][2]]
                ...
                newValues = []
                for value in c[1]:
                    newValues.append([s1pos[0] + value[0], s1Pos[1] + value[1], s1Pos[2] + value[2]])
