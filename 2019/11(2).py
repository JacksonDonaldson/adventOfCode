...
while True:
    ...
    x = position[1]
    y = position[0]
    moves[str(x)+ "," + str(y)] = str(instruction[0])
...
##        print("left")
    ...
    try:
        currentColor = int(moves[str(x)+ "," + str(y)])
    except:
        currentColor = 0
    painted +=1
...
places = []
for move in moves:
    places.append(move.split(","))

from graphics import *
win = GraphWin(title = "j", width = 500, height = 700)
for place in places:
    x = int(place[0]) + 5
    y = int(place[1]) + 10
    p = Rectangle(Point(x * 5, y * 5), Point(x * 5 + 5, y * 5+ 5))
    if moves[str(place[0]) + "," + str(place[1])] == "0":
        p.setFill("black")
    p.draw(win)
