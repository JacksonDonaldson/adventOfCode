...
while True:
    ...
    relY = y+offsetY;
    relX = x+offsetX;
    if(relY> size-1 or relX > size-1):
        #robot trying to leave the currently existing space to right/up, so add to it
        board = add("end", board);
    if( relY<0 or relX<0):
        #robot trying to leave space from left/bottom, so we have to add to space
        #from front, and we have to offset the relativeX/y, to stay in cartesian mode
        board = add("front", board)
        offsetY+=1
        relY+=1
        offsetX+=1
        relX+=1
    board[relY][relX] = instruction[0]
    position[0] -=1
    if(position[0] < -10):
        break
